...
create or replace procedure buscaJugador(nom varchar2) is
    cursor c_jugador is select * from jugador where nombre like('%' || nom || '%');
    total int:=0;
BEGIN
    dbms_output.put_line('Codi        Nom        Equip        Ciutat');
    dbms_output.put_line('**********************************************');
    for jugador in c_jugador loop
        dbms_output.put_line(jugador.codigo || '        ' || jugador.nombre || '        ' || jugador.nombre_equipo  || '        ' || jugador.procedencia);
        total:= total + 1;
    end loop;
    dbms_output.put_line('');
    dbms_output.put_line('Total jugadors mostrat: ' || total);
end;

/* Requeriment 2 */
create or replace procedure JugadorsCiutat(ciutat varchar2) is
    cursor c_ciutat is select * from jugador where nombre_equipo in (select nombre from equipo where ciudad = ciutat);
begin
    dbms_output.put_line('Nom                Equip');
    dbms_output.put_line('*************************');
    for jugador in c_ciutat loop
        dbms_output.put_line(jugador.nombre || '                ' || jugador.nombre_equipo);
    end loop;
end;
...
create or replace trigger trig_audit_jugador
    after insert or update or delete on jugador for each row
begin
    if inserting then
